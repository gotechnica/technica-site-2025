...
<template>
  <VueFinalModal
    class="event-modal"
    :content-class="`event-modal-content ${event?.category}-event`"
  ...
  >
    ...
    <div class="event-modal-body">
      ...
      <div>
        ...
        <a
          v-if="isLink(event?.location)"
          class="event-location-link"
          :href="locationMap.get(event?.location!)"
          target="_blank"
        >
...
function formatTime(startTime?: string, endTime?: string) {
  ...
  const formattedEnd = `${
    formattedStartDay !== formattedEndDay && formattedEndTime !== '12:00 AM'
      ? `${formattedEndDay}, `
      : ''
  }${formattedEndTime}`;
